...
game.Players.PlayerRemoving:Connect(function(player)
	...
	-- Saves the table using the player's UserID as the key --
...
game.Players.PlayerAdded:Connect(function(player)
	player.CharacterAdded:Connect(function(character)
		...
		wait(0.1) -- I changed this from 0.5 to 0.1, though a difference wasn't really noticiable --
		local success, data = pcall(function()
			return playerInventoryData:GetAsync(player.UserId)
		end)

		if success and data then -- Best case scenario, data and saving successful --
			for _, itemName in pairs(data) do
				local toolTemplate = ServerStorage.InventorySaving:FindFirstChild(itemName) -- change "InventorySaving" to the tool folder name in ServerStorage --
